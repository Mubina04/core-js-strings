...
function removeFirstOccurrences(str, value) {
  ...
  if (index !== -1) {
    // Use substring to remove the first occurrence
...
function sumOfCodes(str) {
  ...
  // Use the reduce function to calculate the sum of character codes
  const codeSum = str
    .split('')
...
function countVowels(str) {
  ...
  for (let i = 0; i < strLen.length; i += 1) {
...
function invertCase(str) {
  ...
  arrStr.forEach((letter) => {
    if (letter === letter.toLowerCase()) {
      ...
      newStr += letter; // If the character is not a letter, keep it unchanged
    }
  });
...
function extractNameFromTemplate(value) {
  ...
  // The regular expression captures the name between 'Hello, ' and '!', using (.+?) to match any characters non-greedy.
...
function unbracketTag(str) {
  let res = '';
  if (str.length >= 2) {
    // Use substring or slice to remove the first and last letter
    res = str.substring(1, str.length - 1);
    // Alternatively, you can use str.slice(1, -1);
  } else {
    res = str;
  }
  return res;
...
function extractEmails(str) {
  ...
  // Split the string by semicolons and trim each part
  const parts = str.split(';').map((part) => part.trim());

  // Filter parts to include only valid email addresses
  const validEmails = parts.filter((part) => emailRegex.test(part));
